function [R, history, X, omega, G, Z, chg] = CIMSC(Xo, Po, Xu, Pu, S, G, omega1, lambda, beta,  gamma,  p, c)
% Inputs:
...
% r: the degree of alpha
% b: theta
% p: the index in variable M
...
for iter = 1 : max_iter
    ...
    for v = 1 : num_view
        tmp = X{v}' * X{v};
        Z{v} = ((omega(v) + beta) * eye(N, N) + tmp)\(tmp + omega(v) * R + beta * M(:,:,v));
    ...
    for v = 1 : num_view
        temp_M{v} = (Z{v} - eye(N)) * (Z{v} - eye(N))' + gamma * L;
    ...
    for j=1:n3
        Z1(:,:,j)=0.5*(M(:,:,j)-1/mu*Q1(:,:,j)+W(:,:,j)-1/mu*Q3(:,:,j))-1/mu*(Y(:,:,j).*omega1(:,:,i));
    end
    ...
    for j=1:n3
        ...
        temp2 = Z1(:,:,j)+Q3(:,:,j)/mu;
        linshi_W = temp2-alpha(j)*temp1/mu;
    ...
    temp_M = ((2*beta*R_tensor+mu*Z1+Q1)/(2*beta+mu));
    %        B=Z1+Q1/mu;
    %        Zt_1=permute(Z_t,[2 1 3]);
    %        B_1=permute(B,[2 1 3]);
    [M,~,~] = prox_tnn(temp_M,lambda*beta1/(2*beta+mu),p,mode);
    
    %% Update F
    temp_W=zeros(size(W(:,:,1)));
    for j=1:n3
        temp_W=alpha(j)*W(:,:,j)+temp_W;
    end
    ...
    obj(iter) = history.term1(iter);
     if iter > 2 && abs((obj(iter) - obj(iter - 1)) / obj(iter - 1))< 1e-4 && chg<1e-2
         break;
     end
